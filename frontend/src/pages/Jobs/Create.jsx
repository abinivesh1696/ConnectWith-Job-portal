...
export default function JobCreate() {
  ...
  const [requirementsText, setRequirementsText] = useState(''); // comma-separated
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err?.response?.data?.message || 'Could not post the job opportunity.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto py-2">
      ...
      <div className="glass-panel border border-slate-800/85 rounded-2xl p-6 sm:p-8 shadow-2xl space-y-6">
        <div>
          <h2 className="text-2xl font-extrabold text-white">Post an Opportunity</h2>
          <p className="text-xs text-slate-400 mt-1">Provide clear specifications to attract the right technical candidates.</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5 col-span-2">
              ...
              <input 
                ...
                className="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 focus:border-blue-500 rounded-xl text-slate-200 text-sm placeholder-slate-600 outline-none transition-all focus:ring-2 focus:ring-blue-500/10"
              />
            </div>
          ...
          <div className="space-y-1.5 pt-2">
            ...
            <input 
              ...
              placeholder="e.g. React, Node.js, GraphQL, 3+ years experience"
              className="block w-full px-4 py-2.5 bg-slate-900 border border-slate-800 focus:border-blue-500 rounded-xl text-slate-200 text-sm placeholder-slate-600 outline-none transition-all focus:ring-2 focus:ring-blue-500/10"
            />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <textarea 
              ...
              required
              placeholder="Provide a comprehensive role scope, team workflows, tech stack particulars..."
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full mt-4 py-3 px-4 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white rounded-xl text-sm font-semibold shadow-md shadow-indigo-600/10 active:scale-98 transition-all flex items-center justify-center gap-2"
          >
            {loading ? (
              <>
